...
data %>% ggplot(aes(x = year(begin.date), fill = event.type.major)) +
    geom_histogram(binwidth = 2, na.rm = TRUE, position = "fill") +
...
personal <- data %>%
    ...
    mutate(
        decade = ordered(((year(begin.date)) %/% 10) * 10
                         # , levels = c(50,60,70,80,90,0,10,20,30,40)
                         ),
        event.type.major = ordered(event.type.major, levels = event_levels)
        ) %>%
    group_by(event.type.major, decade) %>%
...
```{r}
personal %>%
    ggplot(aes(x = as.numeric(as.character(decade)), y = victims, color = impact)) +
    geom_line() +
    facet_wrap(~ event.type.major, ncol = 1) +
    scale_y_log10()
```
